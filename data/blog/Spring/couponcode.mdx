...
쿠폰 코드를 대량으로 발급하는 상황에서도 속도 저하 없이 데이터베이스에 `INSERT`를 하기 위해 `JdbcTemplate`를 활용하여 벌크 인서트를 사용하였습니다.

```java
private final int defaultBatchSize = 10000;
private final JdbcTemplate jdbcTemplate;

public void saveAll(List<CouponCode> couponCodeList) {
    String sql = "INSERT INTO coupon_code (coupon_id, client, code, is_registered, created_at, updated_at, expired_at) "
            + "VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    jdbcTemplate.batchUpdate(
            ...
            (PreparedStatement ps, CouponCode couponCode) -> {
                ps.setLong(1, couponCode.getCouponId());
                ps.setString(2, couponCode.getClient().getCode());
                ps.setString(3, couponCode.getCode());
                ps.setBoolean(4, couponCode.getIsRegistered());
                ps.setTimestamp(5, java.sql.Timestamp.valueOf(LocalDateTime.now(ZoneId.of("Asia/Seoul"))));
                ps.setTimestamp(6, java.sql.Timestamp.valueOf(LocalDateTime.now(ZoneId.of("Asia/Seoul"))));
                ps.setTimestamp(7, java.sql.Timestamp.valueOf(couponCode.getExpiredAt()));
            }
    );
